...
class ProcessPowerOutage < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(model)
    ...
    # Specifying a comfort model will make some other values of interest (notably ) available as outputs, particularly with "Pierce"
    ...
    model.getScheduleRulesets.each do |schedule_ruleset|
      ...
      if schedule_ruleset.name.to_s.include?(Constants.ObjectNameHeatingSetpoint)
        if otg_type == "Partial"
          ...
          next #handled with the availability manager below instead
        end
